...
function Avatar({ user, size = "sm" }) {
  ...
  return (
    <div
      className={`${dimensions} rounded-full flex items-center justify-center flex-shrink-0 font-bold text-white ring-2 ring-cyan-400/40`}
...
function ProfilePanel({ user, onLogout, onProfileUpdate, onClose }) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // Clear photo cache for this email on account deletion
      ...
      {!showDeleteConfirm ? (
        <div className="py-2">
          <button
            ...
            disabled={uploading}
            className="w-full flex items-center gap-3 px-4 py-3.5 text-sm text-gray-200 hover:bg-white/10 active:bg-white/20 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3.5 text-sm text-gray-200 hover:bg-white/10 active:bg-white/20 transition-colors"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3.5 text-sm text-red-400 hover:bg-red-500/10 active:bg-red-500/20 transition-colors"
          >
        ...
        <div className="px-4 py-5 space-y-4">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-3 py-2.5 pr-10 rounded-lg bg-gray-700 border border-gray-600 text-white text-sm focus:outline-none focus:border-red-400 transition-colors"
            />
      ...
      <nav className="fixed top-0 left-0 right-0 bg-gray-900/95 backdrop-blur-md border-b border-white/10 z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium">
            {navLinks.map(link => (
              <a key={link.label} href={link.href} className="text-gray-300 hover:text-cyan-400 transition-colors">{link.label}</a>
            ))}
          </div>
            ...
            {user && (
              <div className="flex border-b border-white/10">
                <button
                  ...
                  className={`flex-1 py-3 text-sm font-medium transition-colors ${mobileTab === "menu" ? "text-cyan-400 border-b-2 border-cyan-400" : "text-gray-400"}`}
                >
                  Menu
                </button>
                <button
                  onClick={() => setMobileTab("profile")}
                  className={`flex-1 py-3 text-sm font-medium transition-colors ${mobileTab === "profile" ? "text-cyan-400 border-b-2 border-cyan-400" : "text-gray-400"}`}
                >
                  Profile
                </button>
              </div>
                ...
                <div className="py-4">
                  {navLinks.map(link => (
                    <a
                      ...
                      className="flex items-center px-6 py-4 text-gray-200 hover:text-cyan-400 hover:bg-white/5 transition-colors text-base font-medium"
                    >
                      {link.label}
                    </a>
                  ))}
